...
# ═══════════════════════════════════════════
# 方案2: PowerShell 直接连 CommBridge 读数据
# ═══════════════════════════════════════════
def method2_connect_commbridge():
    print('\n=== 方案2: 直连 CommBridge 探测协议 ===')
    # 尝试用各种格式连 127.0.0.1:53001 (CommBridge 本地端口)
    tests = [
        # (名称, hex请求)
...
# ═══════════════════════════════════════════
# 方案3: 查看 CommBridge 进程的网络 IO 统计
# ═══════════════════════════════════════════
def method3_process_stats():
    print('\n=== 方案3: CommBridge 进程网络统计 ===')
    pid = ps('(Get-Process CommBridge -ErrorAction SilentlyContinue | Select -First 1).Id')
...
# ═══════════════════════════════════════════
# 方案4: 查找 CommBridge 配置和日志文件
# ═══════════════════════════════════════════
def method4_find_configs():
    print('\n=== 方案4: 查找配置和日志 ===')
    # 找 CommBridge 所在目录
    r = ps('(Get-Process CommBridge -ErrorAction SilentlyContinue | Select -First 1).Path')
    commbridge_path = r.strip()
    print(f'  CommBridge 路径: {commbridge_path}')
    if commbridge_path:
        import ntpath
        ...
        # 找日志
        r = ps('Get-ChildItem C:\\ -Recurse -Include CommBridge*.log,CommBridge*.txt,IoMonitor*.log,IoMonitor*.txt -ErrorAction SilentlyContinue -Depth 3 | Select FullName,Length,LastWriteTime | Format-Table -AutoSize')
        print(f'  日志文件:\n{r[:2000]}')


# ═══════════════════════════════════════════
# 方案5: 下载 CommBridge.exe 和 PDB 到本地分析
# ═══════════════════════════════════════════
def method5_download_binary():
    print('\n=== 方案5: 下载 CommBridge 二进制 ===')
    r = ps('(Get-Process CommBridge -ErrorAction SilentlyContinue | Select -First 1).Path')
    path = r.strip()
    if not path:
        print('  CommBridge 未运行!')
        return

    # 获取文件大小
    r = ps(f'(Get-Item "{path}").Length')
    size = int(r.strip()) if r.strip().isdigit() else 0
    print(f'  CommBridge.exe: {size/1024:.0f} KB')
    ...
    if r:
        ...
        with open(os.path.join(local_dir, 'CommBridge.exe'), 'wb') as f:
            f.write(content)
        print(f'  已下载 CommBridge.exe ({len(content)} bytes) 到 {local_dir}')
