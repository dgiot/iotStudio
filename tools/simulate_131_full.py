        ...
        try:
            ...
            while self.running:
                try:
                    ...
                    # 解析 LegacyComm 协议帧: Seq+Flags+Len+Slave+Func+Data
                    if len(data) >= 8:
                        seq = data[0]
                        dlen = data[5]
                        slave = data[6]
...
# ═══════════════════════════════════════════════════
# 2. LegacyComm Server (:53002)
# ═══════════════════════════════════════════════════
class MockLegacyComm:
    """DTU/RTU 桥接服务器"""
    ...
    def start(self):
        ...
        print(f"  [LegacyComm] :{self.port} started")
...
class MockIOServer:
    ...
    def start(self):
        ...
        self.sock.listen(10)
        self.sock.settimeout(1.0)
        print(f"  [IO-Srv] :{self.port} started (RTDB)")
    ...
    def _handle(self, conn, addr):
        ...
        try:
            while self.running:
                ...
                # RTDB protocol: [4B len LE] [4B cmd LE] [payload]
                if len(data) >= 8:
...
def run_unified_collector(cycles=3):
    ...
    for cycle in range(cycles):
        ...
        try:
            ...
            print(f"  [RTDB] :18889 not available: {e}")
...
def main():
    ...
    print("    LegacyComm  :53002      Modbus    :502")
    print("    Modbus      :502 (另)")
    print()
