...
try {
    ...
    Write-Host "REGISTERED: hash=$hash type=$devType ch=$channels"
    ...
    for ($round = 1; $round -le 5; $round++) {
        ...
        if ($st.DataAvailable) {
            ...
            if ($n -ge 8) {
                ...
                Write-Host "QUERY#$round $n B: $reqHex"
                ...
                $final = $respBytes + [byte[]]@(($crc -band 0xFF), (($crc -shr 8) -band 0xFF))
                $st.Write($final, 0, $final.Length)
                Write-Host "  RESPONSE: crc=0x$('{0:X4}' -f $crc)"
            }
        } else {
    ...
    $st.Close(); $c.Close()
    Write-Host "SUCCESS: 5 rounds done"
} catch {
