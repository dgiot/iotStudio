...
db.execute("""INSERT OR IGNORE INTO ontology_site
    ...
    VALUES (?,?,?,?,?,?,?,?)""",
    ("site_dqyt", "DQYT_IO", "OilField", "11.66.12.0/24",
     "pSpace=130:8889, Oracle=129:1521, RTU=191, OPC=5",
     json.dumps({"pspace": "11.66.12.130:8889", "rtu_count": 191}), now, now))
...
db.execute("""INSERT OR IGNORE INTO ontology_gateway
    ...
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)""",
    ("gw_cb_131", "CommBridge_DTU", "11.66.12.131", "site_dqyt",
...
for oid, name, proto, ep, cfg, devs in [
    ...
    ("ch_opc_da", "OPC_DA", "OPC DA DCOM", "172.23.9.3:135", "dcom", "10"),
    ("ch_pspace", "pSpace", "pSpace psAPI", "11.66.12.130:8889", "psapi", "100+"),
    ("ch_dtu", "DTU", "DTU Transparent", ":53001", "serial", "191"),
    ("ch_oracle", "Oracle", "Oracle ADO", "11.66.12.129:1521", "oracle", "0"),
]:
...
devices = [
    ...
    ("02110120089", "DBPA-31A_ATSE89", "ch_opc_da", "DBPA-31A", "OPC", 4),
...
for cid, name, rule, entity, sev, src, action in [
    ...
    ("C_hb", "Heartbeat", "30s超时", "CommBridge", "warn", "IoChannelCfg.ini", "断开重连"),
]:
...
for oid, gw, typ, conn, status, tags, data in [
    ("ds_pspace", "gw_cb_131", "pSpace 6.0.1.9", "11.66.12.130:8889", "connected", 5000,
     json.dumps({"sdk": "6.0.1.9", "exports": 3525, "user": "admin", "handle": "uint16"})),
    ("ds_oracle", "gw_cb_131", "Oracle 11g", "11.66.12.129:1521/DQYTPROD", "connected", 0,
     json.dumps({"tables": ["TAGPAR", "SYS_POINTRELATION"]})),
    ("ds_opc", "gw_cb_131", "Kepware OPC DA", "172.23.9.3:135", "connected", 1000,
...
# Relations
for rid, jdata in [
    ("rel_cb", json.dumps({"source": "02012170058", "target": "gw_cb_131", "label": "monitors", "desc": "RTU->CommBridge"})),
    ("rel_imon", json.dumps({"source": "gw_cb_131", "target": "site_dqyt", "label": "forwards", "desc": "CB->IoMonitor"})),
    ("rel_pspace", json.dumps({"source": "ch_pspace", "target": "ds_pspace", "label": "subscribes", "desc": "IOMan->pSpace"})),
    ("rel_opc", json.dumps({"source": "ds_pspace", "target": "ds_opc", "label": "bridges", "desc": "pSpace->OPC DA"})),
    ("rel_dgiot", json.dumps({"source": "dgiot_lite", "target": "site_dqyt", "label": "replaces", "desc": "dgiot替代IoMonitor"})),
]:
