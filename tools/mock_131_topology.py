...
用 Python socket 在本机复现 131 的完整采集架构:
  ...
  层1: DTU 透传 (:15301) — 模拟 CommBridge + RTU
  层2: OPC DA (:13500)  — 模拟 Kepware OPC Server
  层3: pSpace  (:18889) — 模拟力控 IO Server
...
class MockDTUServer:
    ...
    def start(self):
        ...
        self.sock.listen(10)
        print(f"  [DTU] :{self.port} — CommBridge 模拟")
...
# ═══════════════════════════════════════════════════════════
# 层3: pSpace — 模拟力控 IO Server :18889
# ═══════════════════════════════════════════════════════════
class MockPSpaceServer:
    ...
    def start(self):
        ...
        self.sock.bind(("127.0.0.1", self.port))
        self.sock.listen(10)
        print(f"  [pSpace] :{self.port} — IO Server 模拟")
            ...
            while True:
                ...
                while len(buf) >= 8:
                    ...
                    if cmd == 0x01:  # Connect/Login
                        ...
                        conn.send(resp)
                        pkt(f"<-pSpace", resp, f"LOGIN_OK sid=0x{session_id:04X}")
                        ...
                        conn.send(resp)
                        pkt(f"<-pSpace", resp, f"SUBSCRIBE_OK {tag_count} tags")
                        ...
                        conn.send(resp)
                        pkt(f"<-pSpace", resp, f"READ {len(values)} floats")
        except Exception as e:
            print(f"  [pSpace] {addr} error: {e}")
        finally:
...
def run_collector():
    ...
    try:
        ...
        print("  [pSpace] Connected :18889")
    except:
        ps = None
        print("  [pSpace] :18889 NOT AVAILABLE")
    ...
    for cycle in range(3):
        ...
        if ps:
            try:
                ...
                ps.send(sub)
                pkt(f"pSpace->", sub, "SUBSCRIBE")
                resp = ps.recv(256)
                pkt(f"<-pSpace", resp, "SUB_RESP")
            except: pass
...
def main():
    ...
    print("=" * 60)
    print()
    print("层1: DTU透传 :15301 (CommBridge)")
    print("层2: OPC DA  :13500 (Kepware)")
    print("层3: pSpace  :18889 (IO Server)")
