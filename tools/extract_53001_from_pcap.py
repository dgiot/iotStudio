#!/usr/bin/env python3
"""从 pcapng 提取 CommBridge :53001 真实报文 + 重放到 :53002 测试"""
import sys, os
...
# 按方向分类
outbound = [(p, pl) for p, pl in p53001 if p[TCP].sport == 53001]  # CommBridge → RTU
inbound = [(p, pl) for p, pl in p53001 if p[TCP].dport == 53001]   # RTU → CommBridge

print(f'\nCommBridge→RTU (outbound): {len(outbound)} packets')
print(f'RTU→CommBridge (inbound):  {len(inbound)} packets')
...
# 显示 CommBridge 发出的查询
print(f'\n' + '='*70)
print(f' CommBridge QUERIES (outbound, first 5)')
print('='*70)
for i, (pkt, payload) in enumerate(outbound[:5]):
    dst_ip = pkt[IP].dst
    dst_port = pkt[TCP].dport
    print(f'\n  CommBridge→{dst_ip}:{dst_port} [{len(payload)}B]')
