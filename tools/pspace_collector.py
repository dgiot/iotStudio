#!/usr/bin/env python3
"""
pSpace OPC DA 自主采集器 — 指定 Tag ID 读取实时数据
=====================================================
...
用法:
  ...
  # 读取指定 Tag ID
  C:/Python311-32/python.exe tools/pspace_collector.py --ids 5000,5001,5002

  # 扫描活跃 Tag
  C:/Python311-32/python.exe tools/pspace_collector.py --scan

连接: 11.66.12.130:8889 (pSpace IO Server)
凭据: admin / DQYTA11_pass
"""
import ctypes, os, struct, sys, time, json, argparse
from datetime import datetime

DIR = r"D:\ai\dgiot_lite\plugins\io_server"
...
class PSpaceCollector:
    def __init__(self, host="11.66.12.130", port=8889, user="admin", pwd="DQYTA11_pass"):
        self.host = host
        self.port = port
        self.user = user
        self.pwd = pwd
