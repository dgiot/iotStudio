...
from src.protocols.commbridge_server import LegacyCommServer, DEVICE_TYPES, COEFFICIENTS
        ...
        for row in r.get('rows', []):
            ...
            if devname:
                MIGRATION_PLAN[devname] = {
                    ...
                    'status': 'commbridge',  # 初始都在 LegacyComm
                }
...
async def main():
    print("=" * 70)
    print("  方案C — dgiot_lite LegacyComm TCP Server")
    print(f"  监听: {HOST}:{PORT}")
    print(f"  转发: 192.168.10.131:{PORT} → 11.66.191.155:{PORT}")
