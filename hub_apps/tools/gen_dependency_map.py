...
Usage (inside the hub source tree, or with --src/--dest):
    python3 gen_dependency_map.py [--src /opt/dgiot-4.4/apps] [--dest <app>]
"""
import argparse
import json
import os
...
DEFAULT_SRC = "/opt/dgiot-4.4/apps"
DEFAULT_DEST = os.path.normpath(os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "..", "dgiot_broker"))
...
def scan():
    ...
    for root, _dirs, names in os.walk(SRC_ROOT):
        ...
        if not app.startswith("dgiot"):
            continue
        if app == "dgiot_broker":
            continue  # 本 app 就是替代品，不算 dgiot 对 EMQX 的耦合
        for n in names:
...
if __name__ == "__main__":
    ...
    ap.add_argument("--src", default=DEFAULT_SRC,
                    help="hub apps dir (source of truth to scan)")
    ap.add_argument("--dest", default=DEFAULT_DEST,
                    help="dgiot_broker app dir to write doc/ and compat/ into")
    args = ap.parse_args()
    SRC_ROOT = args.src
