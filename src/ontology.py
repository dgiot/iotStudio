...
class DataSource:
    ...
    id: str
    gateway: str
    type: str                         # oracle, tdengine, pspace, eforcecon, sqlite
...
def build_131_ontology() -> OntologyEngine:
    ...
    engine.register(Site(
        id="dqyt_c1", name="大庆采油厂", type="oil_field",
        location="黑龙江省大庆市",
        description="一厂七矿(CY1C7K) + 一厂八矿(CY1C8K)。IO服务器 11.66.12.131(WIN-NJ7VP96R2NM)"
              " + Oracle 11.66.12.129:1521 + pSpace 11.66.11.131:8889"
    ))

    # ── 层2: Gateway (含完整已安装组件) ──
    engine.register(Gateway(
        id="gw_131", ip="11.66.12.131", site="dqyt_c1",
        hostname="WIN-NJ7VP96R2NM",
        os="Windows Server 2016 (10.0.14393)",
        status="online",
        installed={
            "平台": "力控 ForceControl 7.x / IoMonitor v6.0.0.1",
            "守护进程": "psNTService.exe (6服务自动重启/心跳监控)",
            "CommBridge": "v6.x, PID 19240 — 80+ Modbus TCP 到井口RTU ← 主采集入口",
            "IOMan": "workers ×7 — A11 TCP 到 11.66.12.130:8889 ← 功图采集",
            ...
            "pSpace Server": "v6.0.1.9 @ pSpaceServer64.exe (已停用)",
        },
        channels=["ch_modbus_tcp","ch_a11_rtu","ch_oracle","ch_opc_da","ch_pspace",
                   "ch_eforcecon","ch_redundancy","ch_dtu_pool",
                   "ch_s7","ch_mitsubishi","ch_beckhoff","ch_omron","ch_ge"],
        notes="现场采集两大入口: CommBridge(Modbus TCP :53001→80+RTU) + IOMan(A11 :8889→130)。"
    ...
    channels = [
        ...
        Channel(id="ch_opc_da", gateway="gw_131", name="OPC DA Client",
            protocol="opc_da", endpoint="DCOM :135 → 172.23.9.23/.3/.18.194/.26.6.3",
        ...
        Channel(id="ch_a11_rtu", gateway="gw_131", name="A11 RTU 功图采集",
            protocol="a11_tcp", endpoint="TCP → 11.66.12.130:8889",
            status="running", config={
        ...
        Channel(id="ch_oracle", gateway="gw_131", name="Oracle 数据出口",
            protocol="oracle_sql", endpoint="11.66.12.129:1521/orcl",
            status="running", config={
                "connection": "Provider=OraOLEDB.Oracle.1;User ID=DQYTPROD;Data Source=orcl",
                "password": "DQYTA11_pass (from DataSource.ini)",
        ...
        Channel(id="ch_pspace", gateway="gw_131", name="pSpace 实时库",
            protocol="pspace", endpoint="11.66.11.131:8889",
            status="stopped", config={
                "server": "pSpaceServer64.exe v6.0.1.9",
                "api": "pSpaceAPI.dll (313KB)",
        ...
        Channel(id="ch_redundancy", gateway="gw_131", name="冗余通道",
            protocol="redundancy", endpoint="196.168.65.102:6000/6001",
            status="running", config={
                "partner_ip": "196.168.65.102",
        ...
        Channel(id="ch_dtu_pool", gateway="gw_131", name="DTU协议池 (16种)",
            ...
            status="running", config={
                "drivers": {
                    "DTU_SUNWAY": "三维力控动态IP", "DTU_SUNWAY_COMMSERVER": "通用TCP Server",
                    ...
                    "DTU_DLHB_HJT212": "HJ/T212国标", "DTU_DQQY": "大庆庆远",
    ...
    well_rtus = [
        "K1_51","M5","S21","Y9065","Y9371","Y9721","Y9831","Y9832",
    ...
    for well_id in well_rtus:
        engine.register(Device(
            ...
            manufacturer="大庆油田", model="A11 RTU",
            status="online",
        ))
    ...
    for i in range(1, 19):
        engine.register(Device(
            ...
            manufacturer="力控", model="FORCE_HLS_SIM",
            status="offline",
        ))
    ...
    sample_points = [
        ...
        # 抽油机井测点示例
        Point(id="pt_tgp", device="dev_well_K1_51", name="套压 TGP", unit="MPa",
              register={"protocol":"modbus_tcp","path":"/CY1C8K/B1V354V611/JD010804B1V354V611TGP"},
              alarm={"high":25.0}, category="遥测", description="套管压力"),
        Point(id="pt_zwg", device="dev_well_K1_51", name="总无功 ZWG", unit="kVar",
              register={"protocol":"modbus_tcp","path":"/CY1C8K/B1V354V611/JD010804B1V354V611ZWG"},
              alarm={}, category="遥测"),
        Point(id="pt_zygx", device="dev_well_K1_51", name="总有功 ZYGX", unit="kW",
              register={"protocol":"modbus_tcp","path":"/CY1C8K/B1V354V611/JD010804B1V354V611ZYGX"},
              alarm={}, category="遥测"),
        Point(id="pt_zhl", device="dev_well_K1_51", name="总回流 ZHL", unit="t/d",
              register={"protocol":"modbus_tcp","path":"/CY1C8K/B1V354V611/JD010804B1V354V611ZHL"},
              alarm={}, category="遥测"),
    ]
    ...
    datasources = [
        DataSource(id="ds_oracle", gateway="gw_131", type="oracle",
            connection="11.66.12.129:1521/orcl (DQYTPROD)",
            status="online", tag_count=4_814_742),
        DataSource(id="ds_pspace", gateway="gw_131", type="pspace",
            connection="11.66.11.131:8889",
            status="stopped", tag_count=500),
        DataSource(id="ds_redundancy", gateway="gw_131", type="redundancy",
            connection="196.168.65.102:6000/6001",
