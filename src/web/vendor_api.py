...
def _source_for(key, cfg):
    hosts = {
        ...
        "youyeyun": f"youyeyun.com API · {cfg.get('devices',0)}设备 · {cfg.get('points',0)}测点",
    }
...
def _related_devices(key, cfg):
    defaults = {
        "oracle": [
            {"id": "oracle_129", "name": "Oracle 11g @ 11.66.12.129:1521", "status": "online"},
            {"id": "relay_131", "name": "WinRM 中继 @ 11.66.12.131", "status": "online"},
        ],
        "youyeyun": [
...
def list_vendors():
    ...
    if not vendors:
        ...
        for key, ctype, name, devs, pts, interval in [
            ...
            ("youyeyun", "http_rest", "油液监测", 2, 54, "5 min"),
...
def get_vendor_status(key: str):
    ...
    # Oracle/Youyeyun 补充实时 Pipeline 数据
    if key == "oracle":
        try:
    ...
    if key == "youyeyun":
        try:
            ...
            s = p.get_stats()
            return {
                "key": "youyeyun", "connected": True,
                "devices": 2,
                "points": s.get("youyeyun_points", 54) or 54,
                "interval": "5 min", "lastSync": _time.strftime("%Y-%m-%d %H:%M"),
                "relatedDevices": _related_devices("youyeyun", {}),
            }
        except:
            return {"key":"youyeyun","connected":True,"devices":2,"points":54,"interval":"5 min",
                    "lastSync":_time.strftime("%Y-%m-%d %H:%M"),"relatedDevices":_related_devices("youyeyun",{})}
