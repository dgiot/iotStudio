<template>
  <div v-show="path == 'Userinfo'">
    ...
    />
    <ul v-if="showThemeSetting" class="dgiot-theme-setting">
      <li @click="handleOpenTheme">
        <a>
          <dgiot-icon icon="brush-2-line" />
          <p>{{ $translateTitle('主题配置') }}</p>
        </a>
      </li>
      <li @click="randomTheme">
        <a>
          <dgiot-icon icon="apps-line" />
          <p>{{ $translateTitle('随机换肤') }}</p>
        </a>
      </li>
      <li @click="removeLocalStorage">
        <a>
          <dgiot-icon icon="delete-bin-4-line" />
          <p>
            {{ $translateTitle('清理缓存') }}
          </p>
        </a>
