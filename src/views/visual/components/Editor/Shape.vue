...
<script>
  ...
  import calculateComponentPositonAndSize from '@/views/visual/utils/calculateComponentPositonAndSize'
  import { mod360 } from '@/views/visual/utils/translate'
    ...
    methods: {
      ...
      handleRotate(e) {
        ...
        const move = (moveEvent) => {
          ...
          // 修改当前组件样式
          this.$store.commit('visual/setShapeStyle', pos)
        }

        const up = () => {
          hasMove && this.$store.commit('visual/recordSnapshot')
      ...
      handleMouseDownOnPoint(point, e) {
        ...
        const move = (moveEvent) => {
          ...
          this.$store.commit('visual/setShapeStyle', style)
        }

        const up = () => {
          document.removeEventListener('mousemove', move)
          document.removeEventListener('mouseup', up)
          needSave && this.$store.commit('visual/recordSnapshot')
        }
