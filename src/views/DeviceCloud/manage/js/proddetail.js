...
export default {
  ...
  data() {
    ...
    return {
      viewForm : {
        ...
        data : {
          ...
          initApi : {
            ...
            headers : {store : "localStorage", dgiotReplace : "parse_objectid"},
            dataType : "json"
          },
          body : [ {
            type : "form",
            api : {
              ...
              url : "iotapi/classes/Device/parse_objectid",
              headers :
                  {store : "localStorage", dgiotReplace : "parse_objectid"},
              dataType : "json",
              requestAdaptor :
                  "return {\r\n    ...api,\r\n    data: {\r\n        basedata:{ ...api.data}\r\n    }\r\n}"
            },
            body : [ {type : "input-text", label : "设备名称", name : "name"} ]
          } ]
        },
      ...
      // tabsChild: 'events',
      modules : {
        data : {properties : [], events : [], services : [], tags : []},
        type : "events",
        disabled : false,
        visible : false,
        events : {
      ...
      options : [
        ...
        {value : "last", label : "最后一轮"}, {value : "all", label : "全部"}
      ],
      activeIndex : "",
      editableTabsValue : "1",
      editableTabs : [],
      tabIndex : 1,
      sizeOption : [
        {label : "20", val : "20"}, {label : "不采集(计算值)", val : "计算值"},
        {label : "不采集(主动上报)", val : "主动上报"}
      ],
      // topic数据
      showList : false,
      ...
      topicrule : {
        ...
        topic :
            [ {required : true, message : "请输入Topic类", trigger : "blur"} ],
        type : [
      ...
      sizerule : {
        ...
        identifier :
            [ {required : true, message : "请输入标识符", trigger : "blur"} ],
        dis :
            [ {required : true, message : "请输入数据标识", trigger : "blur"} ],
        type : [
      ...
      structrule : {
        ...
        step : [
          {required : true, trigger : "blur", validator : vailstructspecs}
        ],
        startnumber : [
      ...
      form : {Productname : "", ProductKey : "", ProductAll : 0, data : {}},
      ProductSecret : "",
      dynamicReg : false,
      productId : "",
      productName : "",
      ...
      gridData : [],
      formInline : {name : "", version : "", desc : "", resource : false},
      // objectId: '',
      option : [],
      CategoryKey : "",
      productstart : 0,
      ...
      ace_editor : "",
      iframeShow : false,
      atbas : {visible : false, childrenDrawer : false},
      editIndex : -1,
      eventType : "add",
      formType : "output",
      thingType : "events",
  ...
  computed: {
    treeData() {
      const cloneData =
          JSON.parse(JSON.stringify(this.option)); // 对源数据深度克隆
      // dgiotlog.log("cloneData", cloneData)
      return cloneData.filter((father) => {
        const branchArr = cloneData.filter(
            (child) => father.id == child.parentid); // 返回每一项的子级数组
        branchArr.length > 0
    ...
    ...mapGetters({sizeForm : "konva/sizeForm"})
  },
  watch: {
    topicKey: {
      handler:
    ...
    issub: {
      ...
      handler(val) {
        this.subtimer = window.setInterval(
            () => { this.subAce("formInline", false); }, 5000);
      }
    }
  },
  mounted() {
    ...
    if (this.$route.query.id)
      this.queryProductInfo(this.$route.query.id); // 获取产品信息
  },
  beforeDestroy() {
  ...
  methods: {
    ...
    thingParameters(form, index, type) {
      ...
      console.log("thingParameters", form, this.modules,
    ...
    showDrawer() { this.atbas.visible = true;},
    onClose() { this.atbas.visible = false;},
    showChildrenDrawer() {
      this.eventForm = {
        name : "",
        unit : "/",
    ...
    clearfix(type) {
      ...
      type == "services" ? this.$refs.services.resetFields()
    ...
    handleClose() {
      this.modules.type == "events"
          ...
          : "";
      this.modules.type == "services" ? this.$refs.services.clearValidate() &&
                                            this.$refs.services.resetFields()
                                      : "";
      this.modules.events.data.types = "info";
      this.modules.services.data.transfer = "sync";
      this.modules.visible = false;
      if (this.modules.disabled)
        this.saveExpand(this.productObj.thing);
    },
    async saveExpand(_thing) {
      const res = await putProduct(this.productObj.objectId, {thing : _thing});
      if (res.updatedAt) {
        this.$message({
          type : "success",
          message : "更新成功",
    ...
    async queryProductInfo(productId) {
      ...
      const setThing = _.merge(
          {properties : [], events : [], services : [], tags : []}, thing);
    ...
    async submitModules(type, form) {
      ...
      form.data.output.forEach((o, index) => {
        form.data.outputParams.push({identifier : o.identifier, index : index});
      });
      const _item = {
        ...form.data,
      ...
      this.$refs[type].validate(async (valid) => {
        if (valid) {
          ...
          this.atbas.visible = false;
          this.modules.visible = false;
          // await this.queryProductInfo(this.productObj.objectId) //
          // 更新产品信息
          this.handleClose();
        } else {
    ...
    // },
    ...mapMutations({setSizeForm : "konva/setSizeForm"}),
    ...
    getFormOrginalData() {
      return {
        ...
        precision : 3,
        round : "all",
        struct : [ {attribute : "", attributevalue : ""} ],
        das : [],
        daslist : [],
        rate : 1,
        offset : 0,
    ...
    async handleChildClick(e) {
      await this.queryProductInfo(this.$route.query.id);
      const {properties = [], events = [], services = [], tags = []} =
        ...
        this.viewForm = {
          ...
          data : {
            ...
            body : [ {
              ...
              body :
                  [ {type : "input-text", label : "设备名称", name : "name"} ]
            } ]
          },
          hiddenRow : []
        };
    ...
    async exportProduct(productName) {
      try {
        ...
        dgiotlog.log(res);
        this.$baseMessage(
            this.$translateTitle("alert.Data request successfully"), "success",
            "dgiot-hey-message-success");
        loading.close();
        this.$downBinary(res);
      } catch (error) {
        dgiotlog.log(error);
        this.$baseMessage(this.$translateTitle("alert.Data request error") +
                              `${error}`,
     ...
     * @return {Promise<void>}
     * @document
     * http://prod.iotn2n.com/swagger/#/ProductTemplet/post_classes_producttemplet
     */
        ...
        config = {
          konva : {
            Stage : {
              ...
              children : [ {
                attrs : {id : "Layer_5SKeJT"},
                children : [
                  {
                    attrs : {id : "Group_RoT", width : 2000, height : 2000},
                    children : [],
                    className : "Group"
                  },
                  {
                    attrs : {id : "Group_QtGicp", draggable : true},
                    children : [
                      {
                        attrs : {
                          x : 10,
                          ...
                          data :
                              "M73.142857 0h877.714286v877.714286H73.142857z",
                          fill : "#FFFFFF",
                          angle : -30,
                          scaleX : 0.15,
                          scaleY : 0.15,
      ...
      try {
        ...
        if (res) {
          this.$baseMessage(
              ...
              "success", "dgiot-hey-message-success");
        } else {
          this.$baseMessage(
        ...
        dgiotlog.log(error);
        loading.close();
        this.$baseMessage(this.$translateTitle("user.Save the template error"),
                          "error", "dgiot-hey-message-error");
      }
    },
    // 热加载弹窗
    async updatesubdialog() {
      ...
      getChannelCountByProduct(this.channellength, this.channelstart, product,
                               type)
          .then((res) => {
    ...
    showAllChannel(label = "") {
      ...
      getChannelCountByProduct(
          ...
          label != "" ? this.allChannelstart : this.channelstart, product, type)
          .then((res) => {
            this.allChanneltotal = res.count;
            this.allchannelData = res.results;
          })
          .catch((e) => { dgiotlog.log(e); });
    },
    // 物接入
    getProductChannel() {
      const type = {$in : [ "1", "3" ]};
    ...
    addData() {
      ...
      const params = {config : channelrow};
      this.$update_object("Channel", this.rproductdetailesourcechannelid,
                          params)
          .then(
              (response) => {
                if (response) {
    ...
    addDomain() {
      this.sizeForm.struct.push({attribute : "", attributevalue : ""});
    },
    addDas() { this.sizeForm.daslist.push({addr : ""});},
    removeDas(item) {
    ...
    addDomain1() { this.structform.specs.push({value : "", name : ""});},
    // 物模型提交
    submitForm(obj) {
      dgiotlog.log("obj", obj);
      let data = {item : obj, productid : this.productId};
      // 检测到
      if (this.wmxSituation == "新增") {
        ...
        postThing(data).then((res) => {
          ...
          if (res.code == 200) {
            ...
            this.$message({type : "warning", message : "新增失败" + res.msg});
          }
        });
    ...
    async wmxDataFill(rowData, index, moduletype) {
      ...
      var obj = {};
      var daslist = [];
      rowData.dataType.das.forEach((val) => { daslist.push({addr : val}); });
      // 提交之前需要先判断类型
      if ([ "float", "double", "int", "long" ].indexOf(rowData.dataType.type) !=
          -1) {
        obj = {
          name : rowData.name,
        ...
        obj = {
          ...
          control : rowData.dataForm == undefined ? ""
                                                  : rowData.dataForm.control,
          strategy : rowData.dataForm == undefined ? ""
        ...
        for (const key in rowData.dataType.specs) {
          structArray.push(
              {attribute : key, attributevalue : rowData.dataType.specs[key]});
        }
        obj = {
          name : rowData.name,
          devicetype : rowData.devicetype,
      ...
      this.$nextTick(async () => {
        ...
        this.$refs["sizeForm"].resource.disabled =
            rowData.dataForm.protocol.length ? true : false;
            ...
            if (this.$refs["sizeForm"].resource.value === resource.cType) {
              ...
              resource.arr.forEach((i) => {
                if (i.allowCreate) {
                  this.$refs["sizeForm"].dynamicTable(
                      i, "回显", rowData.dataSource[i.showname]);
                }
              });
              ...
              for (var o in rowData.dataSource) {
                for (var j in resource.obj) {
                  if (o === j)
                    resource.obj[o] = rowData.dataSource[j];
                }
              }
        ...
        if (valid) {
          ...
          if (this.structform.type == "float" ||
            ...
            obj = {
              ...
              dataType : {
                type : this.structform.type.toLowerCase(),
                specs : {0 : this.structform.false, 1 : this.structform.true}
              },
              dataForm : {
                address : this.structform.dis,
                data : this.structform.dinumber
            ...
            obj = {
              name : this.structform.name,
              dataType : {type : this.structform.type.toLowerCase()},
              dataForm : {
                address : this.structform.dis,
                data : this.structform.dinumber
              },
    ...
    editStruct(item, index) {
      ...
      if (item.dataType.type == "float" || item.dataType.type == "double" ||
    ...
    // 删除结构体
    deleteStruct(index) { this.sizeForm.struct.splice(index, 1);},
    async preserve(type) {
      const message = type === "json" ? "物模型修改成功" : "物模型状态更新成功";
      const params = type === "json" ? {thing : JSON.parse(editor1.getValue())}
    ...
    checkAddTest() {
      ...
      this.editableTabs.push({
        ...
        leftItemPos : 0,
        content : this.wmxData[0].dataForm == undefined
                      ? ""
    ...
    menuTabClick(item, itemPos) {
      ...
      tabs.forEach((tab, index) => {
        if (tab.name === item.identifier) {
          // dgiotlog.log("tab.name   " + tab.name, "   item.identifier   " +
          // item.identifier, "   index  " + index);
          isAdd = true;
          mIndex = index;
        }
      });
    ...
    onSaveTap(index) {
      ...
      const params = {thing : this.productdetail.thing};
    ...
    async query_notification() {
      ...
      this.$route.query.id ? (info.where.key = {$regex : this.$route.query.id})
                           : "";
      ...
      this.$get_object("Product", this.productId)
          .then((response) => {
            ...
            if (response) {
              ...
              this.productdetail.createdAt =
                  this.utc2beijing(response.createdAt);
              ...
              if (this.productdetail.thing == undefined ||
              ...
              this.wmxDataBk = [];
              this.wmxData =
                  this.productdetail.thing.properties.filter((item, index) => {
                    item.index = index;
    ...
    handelUpdate(event, row) {
      ...
      const params = {dynamicReg : !isopen};
      this.$update_object("Product", this.productId, params)
          .then((resultes) => {
            if (resultes) {
              this.$message({
      ...
      Compile(Base64.encode(editor.getValue()))
          .then((res) => {
            if (res) {
              ...
              this.warningeditror.map((items) => { log += items + "\r\n"; });
              isupdatetrue += log;
    ...
    subAce(formName, istrue) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.$message({type : "warning", message : "输入格式有误"});
        }
      });
    },
    subAce1(formName) {
    ...
    goToDevices() {
      this.$router.push(
          {path : "/roles/thing", query : {productid : this.productId}});
    },
    addcategory() {
      this.originwmx = true;
      this.getPropData();
    ...
    clickmachine(row, _event, _column) {
      if (row.name == "ALL") {
        ...
        this.wmxData =
            this.wmxDataBk.filter((item) => item.devicetype == row.name &&
                                            !item.dataType.identification);
        this.wmxstart = 1;
      }
    ...
    TypeInstall(origin, arr) {
      arr.map((items, index) => {
        if (items.DataType == "enum" || items.DataType == "bool") {
          ...
          origin.push(obj);
        } else if (items.DataType == "double" || items.DataType == "int" ||
          ...
          JSON.parse(items.DataSpecsList).map((children) => {
            if (children.childDataType == "enum" ||
            ...
            } else if (children.childDataType == "double" ||
    ...
    // 添加物模型模板
    addProCategory(row) {
      const params = {where : {type : row.type, "data.key" : "detail"}};
      this.$query_object("Dict", params).then((res) => {
        const results = res.results;
        if (results && results[0].data.Ability) {
          this.TypeInstall(this.productdetail.thing.properties,
    ...
    deletewmx(row) {
      this.$baseConfirm(
          this.$translateTitle(
              "Maintenance.Are you sure you want to delete the current item"),
          this.$translateTitle("Maintenance.Delete reminder"), async () => {
            const params = {productid : this.productId, item : row};
            const res = await deleteThing(params);
            if (res.code == 200) {
              this.$baseMessage(
                  this.$translateTitle("user.successfully deleted"), "success",
                  "dgiot-hey-message-success");
              await this.getProDetail();
              await this.queryProductInfo(this.$route.query.id);
            } else
              this.$baseMessage(this.$translateTitle("user.error deleted") +
                                        res?.msg
                                    ? res.msg
                                    : res,
                                "error", "dgiot-hey-message-error");
    ...
    eventsCurrentChange(val) { this.eventsStart = val;},
    serverSizeChange(val) {
      this.servicesStart = 1;
      this.servicesPageSize = val;
    },
    serverCurrentChange(val) { this.servicesStart = val;},
    tagsSizeChange(val) {
      this.tagsStart = 1;
      this.tagsPageSize = val;
    },
    tagsCurrentChange(val) { this.eventsStart = val;},
    // 订阅日志按钮
    nowtime() {
      ...
      var D =
          ...
          "日  ";
      var h = (date.getHours() + 1 <= 10 ? "0" + date.getHours()
                                         : date.getHours()) +
              ":";
      var m = (date.getMinutes() + 1 <= 10 ? "0" + date.getMinutes()
                                           : date.getMinutes()) +
              ":";
      var s = date.getSeconds() + 1 <= 10 ? "0" + date.getSeconds()
                                          : date.getSeconds();
      return h + m + s + " ";
    },
    // 订阅日志
    subProTopic(row) {
      ...
      this.topicKey =
          this.$dgiotBus.topicKey(this.mqtt.router, this.mqtt.subtopic);
    },
    mqttMsg(Msg) {
      console.error(Msg);
      this.msgList.push({timestamp : moment().format("x"), msg : Msg});
    ...
    subTopic(formName, isupdated) {
      this.$refs[formName].validate(async (valid) => {
        ...
        if (valid) {
          ...
          await putProduct(this.productId, {topics : this.topicDefault});
          this.$message({
            showClose : true,
            duration : 2000,
            type : "success",
          ...
          this.topicform = {isupdated : -1, value : "", info : "basic"};
    ...
    topicCurrentChange(val) { this.topicstart = val;},
    ...
    deletetopic(topicData, scope, index) {
      this.$baseConfirm("你确定要删除当前项topic吗", null, async () => {
        ...
        topicData.forEach(i => { topic[i.id] = i.value; });
        console.log(this.topicDefault);
        await putProduct(this.productId, {topics : topic});
        this.$message({
          showClose : true,
          duration : 2000,
          type : "success",
    ...
    addEngine() {
      this.$router.push({
        ...
        query : {title : "新增", productid : this.productId}
      });
    },
    actions(row) {
      var string = [];
      row.map((items) => { string.push(items.name); });
      return string.join(",");
    },
    // 表格单个单元格class添加
    ...
    detailRules(id) {
      this.$router.push(
          {path : "/rules_engine/checkengine", query : {id : id}});
    }
  }
};
