...
log = logging.getLogger("vendor_oilmon")

SYS_BASE = "https://www.vendor_oilmon.com/zhc/system"
LOGIN_URL = "https://www.vendor_oilmon.com/zhc/login"


@dataclass
class vendor_aConfig:
    username: str = "CHANGEME_EMAIL"
...
class vendor_aAdapter:
    """有叶云协议适配器 — 自动登录 + 双认证"""

    def __init__(self, config: vendor_aConfig):
        self.config = config
    ...
    def login(self) -> bool:
        ...
        try:
            ...
            d = r.json()
            if d.get("code") != 200:
                log.error(f"[vendor_oilmon] Login failed: {d.get('msg')}")
                return False
            self._set_auth_header(d["token"])
            log.info(f"[vendor_oilmon] Login OK, token: {d['token'][:30]}...")
            return True
        except Exception as e:
            log.error(f"[vendor_oilmon] Login error: {e}")
            return False
        ...
        try:
            ...
            if d.get("code") != 200:
                ...
                if "token" in str(d.get("msg", "")).lower() or "登录" in str(d.get("msg", "")):
                    log.info(f"[vendor_oilmon] Token expired, re-login...")
                    if self.login():
                        r = self.session.get(url, params=params, timeout=30)
                        d = r.json()
                if d.get("code") != 200:
...
def create_adapter(token: str = "", device_id: str = "", name: str = "") -> vendor_aAdapter:
    return vendor_aAdapter(vendor_aConfig(
...
class vendor_aProtocolAdapter(_BaseAdapter):
    ...
    def __init__(self, config: _PConfig):
        ...
        self._yy = vendor_aAdapter(vendor_aConfig(
            username=config.extra.get("username", "CHANGEME_EMAIL"),
...
try:
    ...
    _p_register("vendor_oilmon", version="2.1", category="protocol",
                adapter="vendor_aProtocolAdapter",
                config={
                    "username": "CHANGEME_EMAIL",
...
try:
    ...
    @protocol('http_rest_vendor_oilmon', version='2.1')
    class _vendor_aChannel(BaseChannel):
        def __init__(self, cid, cfg=None): super().__init__(cid, cfg); self._adapter = None
        async def init(self, **kw):
            self._adapter = vendor_aAdapter(vendor_aConfig(**(self.config or {})))
