...
复用已有解析器 (零重复实现):
  ...
  - CommBridge→ parse_response / parse_reg_values (commbridge_server.py)
...
class ProtocolDecoder:
    """统一协议解码器 — A11 / Modbus TCP / CommBridge / IEC104"""
    ...
    def _decode_commbridge(self, frame, device: str) -> List[DecodedPoint]:
        """CommBridge — 复用 parse_response / parse_reg_values"""
        ...
        if vals and vals.get("values"):
            ...
            pts.append(DecodedPoint(ts=frame.ts, device_id=device,
                                    ...
                                    value=0.0, quality=192, protocol="CommBridge",
...
def _selftest():
    ...
    dec.on_frame(_frame("CommBridge", cb_frame, device_port=53001))
    ...
    assert by_proto.get("A11") == 1 and by_proto.get("CommBridge") >= 1
