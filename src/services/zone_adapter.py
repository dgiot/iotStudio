...
对标需求: 每个 IO 服务器 = 一个作业区适配单元
  ...
  + Modbus 动态感知 (RTU点位识别) + IO 本机业务 (Oracle/RTDB节拍)
...
class OpcAdapter:
    ...
    流程: DCOM 连接 → Browse tag 树 → 读取核心点 → 数值与 RTDB 比对
    """
    ...
    def compare_with_rtdb(self, rtdb_values: dict) -> dict:
        """直采值与 RTDB 数值比对 (一致性验证)"""
        results = []
        for it, pv in rtdb_values.items():
            ...
            if isinstance(pv, (int, float)) and isinstance(direct["value"], (int, float)):
                ...
                results.append({"item": it, "rtdb": pv, "direct": direct["value"],
...
def _selftest():
    ...
    cfg = ZoneConfig(
        ...
        opc_adapters=[
            {"vendor": "rockwell", "endpoint": "192.168.10.23"},
            {"vendor": "rockwell", "endpoint": "192.168.10.23"},
            {"vendor": "siemens", "endpoint": "172.26.6.3"},
        ],
        modbus={"rtu_range": "11.248.195-205", "model": "G1-G8_standard",
                "expected_rtu": 191},
        io_business={"oracle_dsn": "INDUSTRYPROD", "commit_span": 300,
    ...
    opc_cfg = {"vendor": "rockwell", "endpoint": "192.168.10.23",
