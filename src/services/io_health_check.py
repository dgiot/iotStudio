...
体检 8 维度 (全部只读, 对标 IO服务器采集规律与品性分析.md):
  ...
  3. 进程健康   — LegacyComm/IOMan/IoMonitor/IoCommit 关键进程存在
...
用法:
  checker = IOHostHealthChecker(ip="192.168.10.131", winrm_config={...})
...
class IOHostHealthChecker:
    ...
    def _check_process(self, data: dict) -> HealthItem:
        ...
        if not missing:
            return HealthItem("进程", "GREEN", 100,
                              "LegacyComm/IOMan/IoMonitor/IoCommit 全部在",
                              {"expected": expected})
        if len(missing) <= 1:
    ...
    def _check_ports(self, data: dict) -> HealthItem:
        ...
        expected = {"53001": "LegacyComm", "8889": "A11",
...
def _selftest():
    ...
    r1 = IOHostHealthChecker("192.168.10.131", "IO-SERVER-131",
    ...
    risky.update({"cpu_pct": 95, "processes": "iomonitor",
                  "ports": "135"})
    r2 = IOHostHealthChecker("192.168.10.130", "IO-130",
    ...
    r3 = IOHostHealthChecker("192.168.10.129", "IO-129",
