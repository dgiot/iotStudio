...
Paho.MQTT = (function (global) {
  ...
  ClientImpl.prototype._connected = function (reconnect, uri) {
    // Execute the onConnected callback if there is one.
    if (this.onConnected) this.onConnected({ reconnect: reconnect, uri: uri })
  }
  ...
  ClientImpl.prototype._reconnect = function () {
    ...
    if (!this.connected) {
      ...
      if (this._reconnectInterval < 60) this._reconnectInterval += 5
  ...
  var Client = function (host, port, path, clientId) {
    ...
    if (arguments.length == 2) {
      ...
      uri =  `${isuseSSL}://` + (ipv6AddSBracket ? '[' + host + ']' : host) + ':' + port + path
    }
    ...
    this.connect = function (connectOptions) {
      ...
      if (connectOptions.mqttVersion > 4 || connectOptions.mqttVersion < 3) {
        throw new Error(
          format(ERROR.INVALID_ARGUMENT, [connectOptions.mqttVersion, 'connectOptions.mqttVersion'])
        )
      }
      ...
      if (connectOptions.reconnectInterval === undefined) connectOptions.reconnectInterval = 10
      ...
      if (connectOptions.hosts) {
        ...
        if (!usingURIs) {
          ...
          for (var i = 0; i < connectOptions.hosts.length; i++) {
            ...
            uri =  `${isuseSSL}://` + + (ipv6 ? '[' + host + ']' : host) + ':' + port + path
