...
async def test_basic():
    ...
    from src.protocols.commbridge_server import CommBridgeServer

    bus = FakeEventBus()
    server = CommBridgeServer(event_bus=bus, port=15301)  # 用非特权端口测试
    await server.start()
...
async def test_multi_rtu():
    ...
    from src.protocols.commbridge_server import CommBridgeServer
    bus = FakeEventBus()
    server = CommBridgeServer(event_bus=bus, port=15302, max_connections=50)
    await server.start()
...
async def test_reconnect():
    ...
    from src.protocols.commbridge_server import CommBridgeServer
    bus = FakeEventBus()
    server = CommBridgeServer(event_bus=bus, port=15303, heartbeat_timeout=5)
    await server.start()
...
if __name__ == "__main__":
    print("=" * 60)
    print("CommBridge TCP Server — 集成测试")
    print("=" * 60)
