...
def t3_auth():
    ...
    for u,p in [("admin","admin123"),("dgiot","dgiot123"),("dgiot_dev","dgiot_dev")]:
...
def t6_infra():
    ...
    try: r=requests.post("http://127.0.0.1:8000/api/auth/login",json={"username":"admin","password":"admin123"},timeout=5);c("API-login",r.status_code==200)
...
def main():
    ...
    try:
        ...
        if s.connect_ex(('127.0.0.1', 7432)) != 0:
            ...
            subprocess.run(['wsl','-d','Kylin-DMZ','-u','root','--','echo','up'],capture_output=True,timeout=15)
