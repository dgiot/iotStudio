#!/usr/bin/env python3
"""
LegacyComm 桥接测试器 — 从 131 直连现场 RTU
===========================================
LegacyComm.exe (PID 19240) 是 Modbus TCP 网关，运行在 131:53001。
RTU 主动连到 53001，LegacyComm 再通过这个连接发送 Modbus 读命令。
...
# ═══════════════════ 1. 状态 ═══════════════════
def check_status():
    print("=== LegacyComm 进程状态 ===")
    r = ps("Get-Process -Name 'LegacyComm*' -ErrorAction SilentlyContinue | Select Id,StartTime,WorkingSet | Format-List")
    print(r[:500] if r else "LegacyComm 未运行")
