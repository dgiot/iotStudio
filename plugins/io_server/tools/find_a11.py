"""
Find more RTDB config files and A11 protocol artifacts
"""
...
# 1. Search for all .ini files containing A11 or LegacyComm references
ps1 = r'''
...
Write-Host "`n=== Search for LegacyComm config ==="
Get-ChildItem -Path $base -Filter "*LegacyComm*" -Recurse -ErrorAction SilentlyContinue | ForEach-Object { Write-Host "$($_.FullName) [$($_.Length)B]" }
'''
run_ps(ps1, "Search A11/LegacyComm files")
...
# 3. Check for LegacyComm channel config (the 3 devices from IoChannelCfg)
ps3 = r'''
$ErrorActionPreference="SilentlyContinue"
