...
REAL_DEVICES = {
    ...
    "DEVICE_ID_PLACEHOLDER": 0x20, "02110150030": 0x30, "DEVICE_ID_PLACEHOLDER2": 0x30,
    ...
    while STATE["running"]:
        try:
            ...
            if cmd == "help":
                ...
                print(f"  LegacyComm :53002  RTU={sum(1 for v in RTU_STATE.values() if v.get('online'))}")
                print(f"  IoMonitor  :9002  records={STATE['records']}")
                print(f"  IoCommit   :9003")
                print(f"  IoProject  :9001")
                print(f"  IO-Srv     :18889 (RTDB)")
...
def main():
    ...
    # 启动服务
    services = [LegacyComm, IoMonitor, IoCommit, IoProject, IOServer, OPCServer, ModbusServer]
