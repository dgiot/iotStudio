"""
CommBridge protocol probe via WinRM - v4
...
# =====================================================================
# S1: Find CommBridge PID and path (use wmi, avoid $pid variable)
# =====================================================================
s1 = r'''
...
# =====================================================================
# S2: CommBridge directory listing using cmd (no PowerShell cmdlets)
# =====================================================================
s2 = r'''
$ErrorActionPreference = "SilentlyContinue"
...
# Step 1: Find CommBridge
run_ps(s1, "S1: Find CommBridge PID & Path")

# Step 2: List directory
run_ps(s2, "S2: CommBridge directory (root only)")

# Step 2b: Read config files in the CommBridge dir (using cmd for listing, GC for content)
run_ps(r'''
$ErrorActionPreference="SilentlyContinue"
$cbid=[int]((netstat -ano|Select-String ":53001"|Select-String "LISTENING")-replace'.*LISTENING\s+(\d+).*','$1')
$p=Get-WmiObject Win32_Process -Filter "ProcessId=$cbid" -ErrorAction SilentlyContinue
