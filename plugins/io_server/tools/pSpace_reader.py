...
RTDB TagID 数据读取器
======================
解析本地 IO ServerOnLine/run/ 目录中的 RTDB/pPluse TagID 文件。
不需要网络连接，直接读本地二进制文件。

用法: python RTDB_reader.py

数据来源:
  D:\ai\io服务器分析\IO ServerOnLine\run\TagID_IOCommitDB*_RTDB.dat
...
def parse_tagid_files():
    """解析所有 RTDB/pPluse TagID 文件"""
    all_tags = []
    for fname in sorted(os.listdir(BASE)):
        if 'RTDB' not in fname and 'pPluse' not in fname:
            continue
        path = os.path.join(BASE, fname)
        with open(path, 'rb') as f:
            text = f.read().decode('gbk', errors='ignore')
        # Extract structured paths
        paths = re.findall(r'/DEVICE_C/[A-Za-z0-9/]+', text) + \
        ...
        for p in paths:
            ...
            if not pt: continue
            all_tags.append({
                'source': 'RTDB' if 'gscyc' in p else 'pPluse',
                'file': fname, 'path': p,
...
def main():
    print("RTDB TagID Data Reader v1.0")
    print(f"Source: {BASE}")
    print()
    ...
    print(f"\nSites: {dict(sites)}")
    print(f"Wells: {len(wells)} (DEVICE_C field)")
    ...
    # Export
    csv_path = os.path.join(os.path.dirname(BASE), '../../docs/RTDB_tags.csv')
