# LegacyComm 专有协议深度分析报告

> 分析日期: 2026-07-12
> 分析对象: LegacyComm.exe (E:\IO ServerOnLine, 192.168.10.131)
...
```
     ...
     IoMonitor.exe (监控)   IoMan.exe (管理)
          |                      |
     WM_COPYDATA IPC        Oracle DB (INDUSTRYPROD)
```
...
| 组件 | 职责 |
|------|------|
| LegacyComm.exe | MFC 主框架, 消息分发, 崩溃处理 |
...
Device.ini 定义了12种油气保护装置，每种都有固定的数据通道映射：
...
LegacyComm 使用 `WM_COPYDATA` 与 IoMonitor 通信（`CMainFrame::OnCopyData`）：
...
LegacyComm 是一个**多协议 GPRS DTU 网关**，核心职能：
...
5. **数据存储**: 通过 ADO 接口写入 Oracle 数据库 (INDUSTRYPROD)
...
| 文件 | 大小 | MD5 |
|------|------|-----|
| LegacyComm.exe | 155,789 | bc015b634cf75f789cf6676404ce58bf |
| LegacyComm.pdb | 484,352 | 0cc2feb050a67f1ea928e907824258a6 |
...
| 文件 | 说明 |
|------|------|
| commbridge_strings_all.txt | LegacyComm.exe 全部字符串 (1053条) |
...
Password=industrya11_PASS
User ID=INDUSTRYPROD
Data Source=orcl
