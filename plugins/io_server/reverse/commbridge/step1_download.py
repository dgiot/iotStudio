"""
Step 1: Download LegacyComm.exe, PDB, and associated DLLs from 192.168.10.131 via WinRM.
"""
...
print("=" * 60)
print("Step 1: Locating and downloading LegacyComm files")
print("=" * 60)

# 1. Get LegacyComm process path
print("\n[1] Finding LegacyComm.exe path...")
ps_out = run_ps(s, "(Get-Process LegacyComm -ErrorAction SilentlyContinue).Path")
if not ps_out.strip():
    # Try alternative
    ps_out = run_ps(s, "Get-WmiObject Win32_Process -Filter \"Name='LegacyComm.exe'\" | Select-Object -ExpandProperty ExecutablePath")
if not ps_out.strip():
    ps_out = run_ps(s, "Get-CimInstance Win32_Process -Filter \"Name='LegacyComm.exe'\" | Select-Object -ExpandProperty ExecutablePath")

exe_path = ps_out.strip().split('\n')[0].strip() if ps_out.strip() else ''
print(f"  LegacyComm.exe path: {exe_path}")

if not exe_path:
    print("  ERROR: Could not find LegacyComm process!")
    # Try directory listing
    dirs_to_try = [
        "C:\\Program Files\\LegacyComm",
        "C:\\Program Files (x86)\\LegacyComm",
        "C:\\LegacyComm",
        "D:\\LegacyComm",
        "C:\\Program Files\\DG\\LegacyComm",
        "C:\\Program Files (x86)\\DG\\LegacyComm",
    ]
    ...
    search_out = run_ps(s, "Get-ChildItem -Path C:\\ -Filter LegacyComm.exe -Recurse -ErrorAction SilentlyContinue | Select-Object FullName")
...
# 3. Download LegacyComm.exe
print(f"\n[3] Downloading LegacyComm.exe...")
download_file_via_certutil(s, exe_path, "LegacyComm.exe")
...
pdb_search = run_ps(s, f"Get-ChildItem -Path C:\\ -Filter LegacyComm*.pdb -Recurse -ErrorAction SilentlyContinue | Select-Object FullName, Length")
...
pdb_path = os.path.join(exe_dir, "LegacyComm.pdb")
pdb_check2 = run_ps(s, f"Test-Path '{pdb_path}'")
if 'True' in pdb_check2:
    print(f"\n  Downloading LegacyComm.pdb...")
    download_file_via_certutil(s, pdb_path, "LegacyComm.pdb")
else:
    print(f"  No LegacyComm.pdb found at {pdb_path}")
