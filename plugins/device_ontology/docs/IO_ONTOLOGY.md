...
| 实体 | 数量 | 关键标识 |
|------|------|----------|
| IO 服务器 | 1台 | 11.66.12.131 (Win2016, E:\IO ServerOnLine\\) |
| pSpace 服务器 | 1台 | 11.66.12.130 (FSmartWorx :80, psAPI :9004, A11 :8889) |
| Oracle 11g | 1台 | 11.66.12.129:1521/orcl (DQYTPROD) |
| RTU (Modbus) | 191台 | 11.248-250.x, 主动连接 :53001 |
| DCS 端点 (OPC DA) | 5台 | 172.23.9.3/.23, 172.26.6.3, 172.23.18.194, 172.21.14.192 |
...
| IoChannelCfg.ini | 3个CommBridge通道 | DEV_COUNT=3 |
...
IoProject.exe (PID 5096)     ← pSpace Core, 总调度
    ...
    │
    ├── IOMan.exe ×24 (IM_A11_RTU)   ← Modbus RTU via CommBridge
...
| 协议 | 帧格式 | 验证 |
|------|--------|------|
| **CommBridge TCP** | Seq(1B)+Flags(4B)+Len(1B)+Slave(1B)+Func(1B)+Data(N) | 95K报文, 异常率0.017% |
...
| 系统 | 认证方式 | 凭据 |
|------|----------|------|
| WinRM :5985 | NTLM | administrator / GKYWB-5991792$1c8k |
| Oracle :1521 | OLEDB | DQYTPROD / dqyta11_PASS |
| pSpace :9004 | psAPI | admin / admin888 |
...
```
CommBridge.exe ←──→ port_TCPServer.dll ←──→ 网卡 11.66.12.131
IOMan.exe      ←──→ psAPISDK.dll       ←──→ 11.66.12.130:9004
IoCommit.exe   ←──→ oraOLEDB            ←──→ 11.66.12.129:1521
...
IoProject.exe (不依赖psAPI — 纯Win32 MFC)
    ...
    │
    ├── IOMan ×18 (type=1, Modbus RTU via CommBridge)
...
关键DLL: iomem.dll (IO端口读写), psAPISDK.dll (pSpace客户端)
```

### 依赖链（根因分析）

```
pSpace Server (130:8889, 必需!)
    ↑ psAPI TCP协议
IoMonitor.exe (可启动 ✅, PID 304, 32MB, 不依赖pSpace启动但无法connect)
    ↑ FindWindow + 共享内存
IoProject.exe (退出code=0 — 找不到IoMonitor窗口时静默退出)
    ↑ CreateProcess
IOMan.exe (退出code=0 — ConnectEx返回-19998 "未初始化")
    ↑ psAPI
pSpace Server ← 无法得知协议帧格式
```

**关键发现**: IoProject.exe/ComeBridge.exe 不依赖 psAPI，可独立运行。IoMonitor.exe 可启动但不连pSpace则无功能。所有 psAPI 依赖项 (IOMan/IoCommit) 需要 pSpace 服务运行才能初始化。

### 本地模拟环境 1:1 映射

| 生产实体 | 本地模拟 | 状态 |
|---------|---------|------|
| CommBridge.exe :53001 | Python CB :53002 | ✅ 真二进制可跑 |
| IoMonitor.exe | Python :9002 | ✅ 真二进制可启动 |
| IoCommit×7 | Python :9003 | ✅ |
| IoProject | Python :9001 | ✅ |
| pSpace :8889 | Python mock :18889 | ✅ 协议模拟 |
...
**结论**: pSpace 使用专有二进制协议，需要 SDK 头文件获取结构体定义才能正确调用。CSDN 上有 6.0.1.9 版 C# 开发包（含头文件/静态库）。在此之前使用纯 Python socket mock。

### 完整设备清单（20台 IOMan 设备 + CommBridge RTU）
...
| ② | dgiot_lite Oracle 管线 | SQL 同步 | 历史 | ❌ 需131 | ✅ 油田数据已通 |
| ③ | psAPISDK.dll → pSpace | 共享内存/TCP | 实时 | ❌ 需130 | ❌ Connect阻塞 |
...
```
本机 127.0.0.1:
  CommBridge  :53002  ← DTU/RTU
  IoMonitor   :9002   ← 数据汇聚
  IoCommit    :9003   ← 数据写入
  OPC DA      :13500  ← OPC 数据源
  pSpace      :18889  ← pSpace 数据源
  Modbus TCP  :502    ← 直连轮询
```
...
```
131 Oracle (DQYTPROD) → WinRM / VBS / ADO → dgiot_lite :8000 /api/devices
```

当前已同步: 274 设备 · 261,488 遥测行 (油田 `oil_field_dqyt`)
...
### 路径③: psAPISDK.dll → pSpace (TCP:8889)
...
**阻塞原因**: pSpace 私有二进制协议, 服务端 130:8889 不发任何握手数据即关闭连接
**解除方法**: 获取 pSpace 6.0.1.9 SDK 头文件 (`psConnectPara` 结构体定义)
**CSDN 链接**: https://blog.csdn.net/websocket5live/article/details/161912273

**关键导出函数**:
```
Net_Init(callback1, callback2, flags)       → 网络层初始化
Net_ConnectServer(psConnectPara*, wchar_t*) → 连接远程pSpace
...
WinRM → 131: IoProject.exe 已连接 IoMonitor(Hwnd:3B6C0B5C)
       ...
       → IOMan 连接 pSpace → 采数据 → WM_COPYDATA → IoMonitor → Oracle
```
...
```
本地 → WinRM → 131 → DCOM(:135) → Kepware(172.23.9.3/9.23, CLSID:6E6170F0-...)
...
| IP | 功能 | CLSID |
|------|------|-------|
| 172.23.9.3 | Kepware OPC DA | 6E6170F0-FF2D-11D2-8087-00105AA8F840 |
| 172.23.9.23 | Kepware OPC DA | 同上 |
| 172.26.6.3 | Kepware OPC DA | 同上 |
| 172.28.5.200 | OPC DA (10+ DCOM连接) | 待确认 |
| 172.23.18.194 | OPC DA | 待确认 |
