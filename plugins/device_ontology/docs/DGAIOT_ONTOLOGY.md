...
│  │ 16 口油井 (DEV_A,M5,S21,Y9065~Y9832,YPing1...)              │  │
...
- 示例: `/DEVICE_D/WELL_001/STATION_01WELL_001GYS`
...
```
LegacyComm.exe (PID 19240)     ← 采集层：Modbus TCP 直连 80+ 井口RTU
...
IOMan workers (PIDs 392/5036/8628/9244/12244/15164/15256) ← 采集层：A11 协议
  └─ → 192.168.10.130:8889 ×7 ESTABLISHED

IoMonitor.exe (PID 18400)      ← 汇聚层：只连数据库
  └─ → Oracle 192.168.10.129:1521 ESTABLISHED (唯一连接!)
...
```
✅ LegacyComm.exe PID 19240 — 80+ Modbus TCP 到井口 (11.248.x.x/11.249.x.x)
✅ IOMan workers ×7 — A11 协议到 192.168.10.130:8889
✅ OPC_FC_Client — OPC DA 采集 (17,344 条 DX/JB 测点)  ← 已确认
✅ IoMonitor → Oracle 192.168.10.129:1521 (唯一连接，纯数据出口)
...
# 测点路径解析
b.parse_point_path("/DEVICE_D/WELL_001/STATION_01WELL_001GYS")
# → {site: DEVICE_D, well: WELL_001, station: STATION_01, point_code: GYS}
...
LegacyComm:    活跃 — 通道编号 385, 80+ Modbus TCP 连接
FORCE_HLS_SIM: 已注册
...
LegacyComm.exe (PID 19240) ─→ Modbus TCP :53001 → 80+ RTU (11.248/11.249)
IOMan workers (×7 PIDs)     ─→ A11 TCP :8889 → 192.168.10.130
        │
        ▼
IoMonitor.exe (PID 18400)   ─→ Oracle 192.168.10.129:1521
        944 活跃点, 5s 周期, 实时写功图 (载荷/电流)
```

**为什么 WinRM OPC 连不上:**
LegacyComm + A11 是 TCP 直连（不需要令牌）。OPC_FC_Client 也是活跃的——但它可能连的是本机/locally bridged 的 OPC Server，不是远程 172.23.9.x。我们通过 WinRM 远程连目标 OPC Server 仍然被 DCOM 拒绝——但这不影响 IoMonitor 内部运行。
...
**数据路径**: `本机 → WinRM(192.168.10.131:5985) → cscript 32位 → VBS/ADO → Oracle(192.168.10.129:1521)`
...
─────────────────────────────────────────────────────────────────────────────────────
Modbus TCP    LegacyComm.exe (19240)  190 RTU   Modbus TCP   11.248/11.249:53001  SYS_POINTRELATION_WELL
OPC DA        IOMan.exe ×5            5 OPC    OPC DA DCOM  172.23/172.26:135     SYS_POINTRELATION_STATION
A11 RTU       IOMan.exe ×7            7 A11    A11 TCP      192.168.10.130:8889     PC_FD_PUMPJACK_FDYNA_DIA_T
─────────────────────────────────────────────────────────────────────────────────────
汇聚层        IoMonitor.exe (18400)   1        Oracle OLEDB 192.168.10.129:1521      CommitRealSpan=300ms
─────────────────────────────────────────────────────────────────────────────────────
总计: 966口井 · 44,977测点 · 4,815,777功图 · 233,462运行记录 · 799在线状态
```

**关键发现:**
- LegacyComm 是纯 Modbus TCP 网关，单端口 53001，专有协议（非标准 Modbus TCP）
- OPC DA 确认活跃 — IOMan.exe ×5 连接 5 台 OPC Server（192.168.10.23/.3/.18.194/.26.6.3/.21.14.192）
- RTU 不监听 502 端口 — 采用 RTU-initiated 反向连接架构
- 数据全部汇入 Oracle — 已导出 10 份 CSV + 4 份 PDF

**工具清单:**
- `oracle_reader.py`    — Oracle 直读 (81行/秒, WinRM中继)
- `RTDB_reader.py`    — RTDB 历史数据解析 (DEVICE_C)
- `protocol_tester.py`  — 三协议状态测试
- `commbridge_tester.py`— LegacyComm 桥接测试
