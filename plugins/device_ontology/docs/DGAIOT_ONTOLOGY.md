...
│  │ CommBridge.exe (通信桥) → SYNCH×1 + CommBridge×3 + TCP×1     │  │
...
│                                                                     │
│  ┌─ 数据出口 (Data Servers) ────────────────────────────────────┐  │
│  │ Oracle @ 11.66.12.129:1521/orcl (DQYTPROD/DQYTA11_pass) ←主 │  │
│  │ pSpace @ 11.66.11.131:8889 (力控实时库) ←辅助               │  │
...
│  │ 11.66.12.129 :1521 Oracle 数据库 (DQYTPROD)                  │  │
│  │ 11.66.11.131 :8889 pSpace 实时库                             │  │
│  │ 196.168.65.102      冗余备机                                  │  │
│  └──────────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────────┘
```
...
| `CommBridge.exe` | 155KB | - | 通信网桥 (TCP/串口) |
| `IoProject.exe` | - | - | 工程管理器 |
| `IoTest.exe` | - | - | 测试工具 |
...
| 服务 | 版本 | 说明 | 心跳(s) | 自动启 |
|------|------|------|---------|--------|
| `pSpaceServer64.exe` | 6.0.1.9 | pSpace 实时库 | 60 | ✅ |
...
- `pSpaceAPI.dll` (313KB) — pSpace 实时库 API
...
| `DTU_SUNWAY` | 三维力控 (动态IP) | Sunway Dynamic IP |
...
#### 9.2.6 数据库表 (from Oracle DQYTPROD)

| 表名 | 行数 | 说明 |
|------|------|------|
| `SYS_POINTRELATION_STATION` | 40,410 | **OPC DA + 注水站测点** (DX 16,375 + JB 969 + Z1 6,202 + Z2 12,612) |
| `PC_FD_PUMPJACK_FDYNA_DIA_T` | 4,814,742 | 抽油机功图诊断 |
| `SYS_DEVICE_RUN_DETAILS_HIST` | 233,269 | 设备运行详情历史 |
| `SYS_POINTRELATION_WELL` | 4,567 | Modbus TCP 测点 (B1V/B2V/B3V) |
| `SYS_SINGLE_WELL_BASE_INFO` | 966 | 单井基础信息 |
| `SYS_DEVICE_ONLINE_DETAILS` | 799 | 设备在线状态 |
| `TOURWELL_RECORD_DETAIL` | 2,134 | 巡井记录 |
| `PROJECT_DEVICEPAR` | 256 | A11 RTU 设备参数 |
| `PROJECT_CHANNELPAR` | 245 | 通道参数 (注水站 Z203/Z204/Z211...) |
| `SYS_USER` | 4 | 系统用户 |

#### 9.2.7 数据提交组 (12 组, from run/TagID_IOCommitDB*.dat)

| 组 | CY1C7K 大小 | CY1C8K 大小 | pPluse 大小 | pSpace |
|----|------------|------------|------------|--------|
...
**测点路径格式:** `/CY1C8K/{WellID}/{DeviceCode}{WellID}{MeasurementType}`
- 示例: `/CY1C8K/B1V354V611/JD010804B1V354V611GYS`
...
现场设备 (RTU/PLC/继电器)
   │
   ├──[Modbus TCP :502]──→ CommBridge.exe ──→ IoMonitor.exe
...
IoMonitor.exe (主采集引擎, CommitRealSpan=300ms)
   ...
   │      │
   │      ├──[ADO/OLEDB]──→ Oracle 11.66.12.129:1521/orcl (DQYTPROD)
   │      │     └── A11SQLSERVICE.exe ──→ F:\TRANgo\...\RTUSql
   │      │
   │      ├──[pSpace API]──→ pSpace 11.66.11.131:8889
   │      │
...
```
WIN-NJ7VP96R2NM (主) ←→ 196.168.65.102 (备)
...
#### 9.5.1 活性证据 (2026-07-12 实时连接快照)

**三层进程架构 — 各司其职:**

```
CommBridge.exe (PID 19240)     ← 采集层：Modbus TCP 直连 80+ 井口RTU
  ├─ :53001 → 11.248.195.x ~ 11.248.205.x (八厂)
  └─ :53001 → 11.249.34.x ~ 11.249.61.x (七厂)

IOMan workers (PIDs 392/5036/8628/9244/12244/15164/15256) ← 采集层：A11 协议
  └─ → 11.66.12.130:8889 ×7 ESTABLISHED

IoMonitor.exe (PID 18400)      ← 汇聚层：只连数据库
  └─ → Oracle 11.66.12.129:1521 ESTABLISHED (唯一连接!)

OPC DA (DCOM :135)             ← OPC_FC_Client 采集，数据在 SYS_POINTRELATION_STATION (17,344条) ✅
  └─ DX/JB/Z1/Z2 前缀测点 → 确认在 Oracle 中
```

**关键发现 (2026-07-12 最终验证):** OPC DA 数据 **确实在 Oracle 中**，存储在 `SYS_POINTRELATION_STATION` 表（40,410行），其中 DX+JB 前缀共 17,344 条为 OPC DA 采集数据。之前误判为"无 OPC 数据"是因为只检查了 SYS_POINTRELATION_WELL 表。

```
✅ CommBridge.exe PID 19240 — 80+ Modbus TCP 到井口 (11.248.x.x/11.249.x.x)
✅ IOMan workers ×7 — A11 协议到 11.66.12.130:8889
✅ OPC_FC_Client — OPC DA 采集 (17,344 条 DX/JB 测点)  ← 已确认
✅ IoMonitor → Oracle 11.66.12.129:1521 (唯一连接，纯数据出口)
✅ 运行率 99.31% @ 2026-07-11 23:50
✅ Oracle 总测点: 44,977 (WELL 4,567 + STATION 40,410)
```
...
⚠️  CommBridge.exe 频繁崩溃 (2023-03 ~ 2026-07)
...
# 测点路径解析
b.parse_point_path("/CY1C8K/B1V354V611/JD010804B1V354V611GYS")
# → {site: CY1C8K, well: B1V354V611, station: JD010804, point_code: GYS}
...
### 9.7 实测数据流终局 (2026-07-12 实时连接 + IoMonitor 运行界面)

> **来源: RDP 桌面截图 — IoMonitor 实时监控界面 + netstat 连接快照**

**IoMonitor 运行界面关键指标:**

```
OPC_FC_Client: 活跃 — JB1V2/DX6PZ/Z22Y/Z1PZ/DX1ZRZ 等井
CommBridge:    活跃 — 通道编号 385, 80+ Modbus TCP 连接
FORCE_HLS_SIM: 已注册

状态: 正常 | 点数: 944 | 活动点: 944
更新周期: 1000ms | 超时: 10000ms | 命令间隔: 100ms
请求次数: 24,696 | 采集超时: 0 | 平均采集周期: 5,004ms
```

**数据库实时写入证据 (IoMonitor 日志):**
```
02204010144_B1VD4VE44: 最大载荷 48.98, 最小载荷 15.47 → 【插入数据库成功】
21001080009_B1V343VS701: 最大载荷 84.14, 最小载荷 9.05 → 【插入数据库成功】
02107010048_B1VD26VSFE32: 最大载荷 67.09 → 【插入数据库成功】
```

**三层进程架构:**

```
OPC_FC_Client (ioapi.dll) ─→ OPC DA 采集 (JB1V2/DX6PZ/Z22Y/Z1PZ/DX1ZRZ)
CommBridge.exe (PID 19240) ─→ Modbus TCP :53001 → 80+ RTU (11.248/11.249)
IOMan workers (×7 PIDs)     ─→ A11 TCP :8889 → 11.66.12.130
        │
        ▼
IoMonitor.exe (PID 18400)   ─→ Oracle 11.66.12.129:1521
        944 活跃点, 5s 周期, 实时写功图 (载荷/电流)
```

**为什么 WinRM OPC 连不上:**
CommBridge + A11 是 TCP 直连（不需要令牌）。OPC_FC_Client 也是活跃的——但它可能连的是本机/locally bridged 的 OPC Server，不是远程 172.23.9.x。我们通过 WinRM 远程连目标 OPC Server 仍然被 DCOM 拒绝——但这不影响 IoMonitor 内部运行。

**结论: IoMonitor 三路采集全活——OPC + Modbus + A11——数据实时进 Oracle。**
...
> 不依赖 dgiot_lite 服务器，直连 Oracle 读取生产数据。

**文件**: `oracle_reader.py` (项目根目录)

**依赖**: `pip install pywinrm`

**用法**: `python oracle_reader.py`

**数据路径**: `本机 → WinRM(11.66.12.131:5985) → cscript 32位 → VBS/ADO → Oracle(11.66.12.129:1521)`

**验证结果 (2026-07-12 04:57)**:
...
**技术说明**:
- 使用 32 位 cscript.exe 调用 ADO (OraOLEDB.Oracle.1) 连接 Oracle
- VBS 脚本通过 base64 分块上传，每次独立文件名避免缓存
- 线程安全，支持并发查询
- 不依赖 dgiot_lite 后端服务，可独立部署
- 所有依赖组件（ADO、Oracle Client、cscript）均已在 131 上预装

---

## 十、三路采集最终架构 (2026-07-12 实时连接快照确认)

```
采集入口            进程                数量      协议          目标                 Oracle表
─────────────────────────────────────────────────────────────────────────────────────
Modbus TCP    CommBridge.exe (19240)  190 RTU   Modbus TCP   11.248/11.249:53001  SYS_POINTRELATION_WELL
OPC DA        IOMan.exe ×5            5 OPC    OPC DA DCOM  172.23/172.26:135     SYS_POINTRELATION_STATION
A11 RTU       IOMan.exe ×7            7 A11    A11 TCP      11.66.12.130:8889     PC_FD_PUMPJACK_FDYNA_DIA_T
─────────────────────────────────────────────────────────────────────────────────────
汇聚层        IoMonitor.exe (18400)   1        Oracle OLEDB 11.66.12.129:1521      CommitRealSpan=300ms
─────────────────────────────────────────────────────────────────────────────────────
总计: 966口井 · 44,977测点 · 4,815,777功图 · 233,462运行记录 · 799在线状态
```

**关键发现:**
- CommBridge 是纯 Modbus TCP 网关，单端口 53001，专有协议（非标准 Modbus TCP）
- OPC DA 确认活跃 — IOMan.exe ×5 连接 5 台 OPC Server（172.23.9.23/.3/.18.194/.26.6.3/.21.14.192）
- RTU 不监听 502 端口 — 采用 RTU-initiated 反向连接架构
- 数据全部汇入 Oracle — 已导出 10 份 CSV + 4 份 PDF

**工具清单:**
- `oracle_reader.py`    — Oracle 直读 (81行/秒, WinRM中继)
- `pSpace_reader.py`    — pSpace 历史数据解析 (CY1C7K)
- `protocol_tester.py`  — 三协议状态测试
- `commbridge_tester.py`— CommBridge 桥接测试
- `rtu_bridge.py`       — RTU 桥接扫描
