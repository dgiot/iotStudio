"""
CommBridge 集成服务 — 连接 Oracle 设备配置 + EventBus + 存储
=============================================================
从 Oracle PROJECT_DEVICEPAR 加载 RTU 设备配置,
注册到 CommBridgeServer, 通过 EventBus 推送遥测数据,
写入 TDengine/SQLite.

用法:
  integration = CommBridgeIntegration(event_bus, oracle_reader)
...
from ..protocols.commbridge_server import (
    CommBridgeServer, DEVICE_TYPES, COEFFICIENTS,
...
class CommBridgeIntegration:
    """CommBridge 集成 — 设备加载 + 数据存储 + MQTT 推送"""
    ...
    def __init__(self, event_bus=None, storage=None, mqtt=None,
        ...
        self._port = port
        self._server = CommBridgeServer(event_bus=event_bus, port=port)
...
class RtuSimulator:
    """模拟 RTU 设备 — 用于测试 CommBridgeServer"""
