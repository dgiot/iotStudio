...
class IOOntology:
    ...
    def load_rules(self):
        ...
        self.rules = [
            ...
            # 通信约束
            {
                'id': 'R003', 'name': 'LegacyComm 通道监控',
                'layer': 'Logic',
                'condition': lambda v: '打开LegacyComm通道失败' in str(v),
                'action': 'ALARM', 'severity': 'ERROR',
                'msg': 'LegacyComm 通道故障: {device}'
            },
            # 采集约束
            {
    ...
    def get_relations(self):
        ...
        return [
            ...
            {'from': 'IOMan×36', 'relation': 'connectsTo', 'to': 'RTDB A11', 'via': 'A11 5a5a :8889'},
            {'from': 'IoProject', 'relation': 'manages', 'to': 'IOMan×36 IoCommit×7 LegacyComm', 'via': 'RTDB SDK'},
            {'from': 'IoCommit×7', 'relation': 'writesTo', 'to': 'Oracle 功图库', 'via': 'TNS :1521'},
            {'from': 'LegacyComm', 'relation': 'manages', 'to': '31 无线终端', 'via': 'GPRS/CDMA'},
...
if __name__ == '__main__':
    ...
    print('\n=== SWRL 规则测试 ===')
    test_events = [
        {'device': 'DEVICE_ID_PLACEHOLDER', 'parameter': '最大载荷', 'value': 86.07},
        {'device': 'DEVICE_ID_PLACEHOLDER2', 'parameter': '最大下行电流', 'value': 15.28},
        {'device': 'LegacyComm-236', 'msg': '打开LegacyComm通道失败，编号236'},
        {'operation': 'install'},
    ]
