...
流水线:
  131(ADB/ADO) → Oracle(129:1521) → OracleBridge → parse_point_path
    → TDengine(172.22.193.167:6041) or SQLite(fallback)
    → MQTT(127.0.0.1:1883)
...
class OraclePipeline:
    ...
    def __init__(self):
        ...
        self._tdengine = None
        self._mqtt = None
        self._youyeyun_adapters = []  # 有叶云适配器列表
        self._stats = {
    ...
    async def start(self):
        ...
        # 初始化有叶云适配器
        try:
            from ..protocols.youyeyun import create_adapter, YouyeyunConfig
            from ..config import cfg
            yy_cfg = cfg.youyeyun if hasattr(cfg, 'youyeyun') else None
            if yy_cfg:
                for dev in (yy_cfg.devices if hasattr(yy_cfg, 'devices') else
                    ...
                    self._youyeyun_adapters.append(adp)
        except ImportError:
        ...
        logger.info(f"[pipeline] Pipeline started (Oracle + {len(self._youyeyun_adapters)} youyeyun devices)")
        return {"status": "started", "tdengine": "fallback" if self._tdengine._is_fallback else "connected",
                "youyeyun_devices": len(self._youyeyun_adapters)}
    ...
    async def _main_loop(self):
        ...
        while self._running:
            try:
                ...
                if tick % 5 == 0 and self._youyeyun_adapters:
                    await self._collect_youyeyun()
    ...
    async def _collect_run_rate(self):
        ...
        await self._tdengine.insert_point(
            device_id="oil_field_dqyt", point_id="run_rate",
            point_name="运行率", value=rate, unit="%",
            device_type="oil_field", station_id="CY1C8K",
            quality=192,
        )

        # 推 MQTT
        self._push_mqtt("dgiot/dqyt_c1/gw_131/ch_oracle/run_rate/data", {
        ...
        try:
            ...
            bus.emit("pipeline.point_written", device_id="oil_field_dqyt",
    ...
    async def _collect_youyeyun(self):
        """采集有叶云油液监测数据 (每 5 分钟)"""
        for adp in self._youyeyun_adapters:
            try:
                ...
                for pt in realtime:
                    ...
                    # 推 MQTT
                    self._push_mqtt(
                        f"dgiot/youyeyun/{adp.config.device_name}/{pt['key_id']}/data",
                ...
                self._stats["youyeyun_points"] += count
                self._stats["points_written"] += count
                self._stats["mqtt_pushed"] += count
    ...
    async def _collect_stats(self):
        ...
        try:
            opcda_result = await self._query_oracle(
                "SELECT "
                "(SELECT COUNT(*) FROM SYS_POINTRELATION_STATION WHERE POINT_LONGNAME LIKE '/CY1C8K/DX%') AS DX, "
                "(SELECT COUNT(*) FROM SYS_POINTRELATION_STATION WHERE POINT_LONGNAME LIKE '/CY1C8K/JB%') AS JB, "
                "(SELECT COUNT(*) FROM SYS_POINTRELATION_STATION WHERE POINT_LONGNAME LIKE '/CY1C8K/Z1%') AS Z1, "
                "(SELECT COUNT(*) FROM SYS_POINTRELATION_STATION WHERE POINT_LONGNAME LIKE '/CY1C8K/Z2%') AS Z2 "
                "FROM dual",
                "opcda_stats"
            )
        ...
        # 推 MQTT
        self._push_mqtt("dgiot/dqyt_c1/gw_131/ch_oracle/stats/data", {
            "ts": datetime.now(timezone.utc).isoformat(),
            "tables": stats,
        })
