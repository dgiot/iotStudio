...
Modbus TCP:  LegacyComm.exe → 80+ RTU → Oracle SYS_POINTRELATION_WELL (4,567点)
Modbus RTU:  Standard_Umodbus → (历史, 当前未运行)
A11:         IOMan workers → 192.168.10.130:8889 → Oracle PC_FD_PUMPJACK_FDYNA_DIA_T (4.8M功图)
...
def test_modbus_tcp():
    section("Modbus TCP — LegacyComm.exe 采集 (DEVICE_D)")
...
def test_a11():
    section("A11 — CNPC 采油厂协议 (IOMan → 192.168.10.130:8889)")
    ...
    # A11 连接状态
    print(f"\n  A11 连接: IOMan workers ×7 → 192.168.10.130:8889")
...
def test_all():
    ...
    print(f"""
  三协议数据对比:
    Modbus TCP: 4,567 测点 (B1V/B2V/B3V) — LegacyComm.exe 采集 — DEVICE_D
    Modbus RTU: 历史残留 — 驱动文件存在但未运行 — 无当前数据
    A11:        4.8M 功图 (256台RTU) — IOMan workers 采集 — 192.168.10.130:8889

  独立工具:
    oracle_reader.py    Oracle 直读 (Modbus TCP + A11 数据)
    RTDB_reader.py    RTDB 历史数据 (DEVICE_C)
